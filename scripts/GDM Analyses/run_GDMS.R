...
loadEnvData = function(morphlocs,morphspp,morphdf,doGENE=F,spp,verbose=F) {
...
doImportanceTesting = function(doImp,model,formatted,distPredList,doPCA,doGENE,spp,doChroms,whichchrom,verbose=F) {
  start_time <- Sys.time()
  ...
  if(model$splines[1] == 3) {
    ...
    modTest <- gdm.varImp(formatted,geo = F,fullModelOnly = T)
    
  }
  end_time <- Sys.time()
  print(end_time - start_time)
  rownames(modTest[[2]])[(length(model$predictors) - 6):length(model$predictors)] = names(distPredList)
  rownames(modTest[[3]])[(length(model$predictors) - 6):length(model$predictors)] = names(distPredList)
  rownames(modTest[[4]])[(length(model$predictors) - 6):length(model$predictors)] = names(distPredList)
  if (doPCA == F) {
    if (doGENE == F) {
      suffix="_allmorph" 
    } else {
...
runGDMs = function(doimp = F, doPCA = F,doGENE = F,whichpca = 1,univariate = T,dogeo = T,doChroms = F,whichchrom="Z",
  ...
  for (spp in spplist) {
    ...
    if(doEnvpcs==F) {
      bioclim_final = bioclim_final[,c("Sample","Latitude","Longitude")]
    }
    ## load matList 
    ...
    for (matVariable in matList) {
      ...
      if(length(csvstring) > 1) {
        ...
        if(matVariable=="STR") {
          df = readDataChangeHeaderGDM(csvstring,makeUnique = T) ## originally F
    ...
    print(paste("Keeping:",length(tokeep),"of",length(originals)+1))
    ...
    for (matVariable in matList) {
      ...
      if(setequal(rownames(toset),colnames(toset)[colnames(toset)!="Sample"])) {
        ...
        if(setequal(toset$Sample,colnames(toset)[colnames(toset)!="Sample"])) {
          ...
          print("SAMPLE AND COLS DON'T MATCH")
        ...
        print("ROWS AND COLS DON'T MATCH")
        #print(rownames(toset))
        #print(colnames(toset))
    ...
    if(univariate==F) {
      ...
      ## remove NA 
      formatted = formatted[complete.cases(formatted),]
      print(paste("ROWS REMAINING:",nrow(formatted),"OF",oldrows))
      ...
      if (!(is.null(model))) {
        if(doEnvpcs==T) {modeloff=7} else {modeloff=1}
      ...
      if(doimp==T) {
        doImportanceTesting(doImp,model,formatted,distPredList,doPCA,doGENE,spp,doChroms,whichchrom)
      }
    } else {
      if(doEnvpcs==T) {colsep=6} else {colsep=0}
...
matList=c("GENE","STR","PRES","IBD","ABUN","ENV")
...
for(spp in spplist) {
  for(i in 2:length(matList)) {
    tempMatlist = matList[c(1,i)]
    print(tempMatlist)
    # runGDMs(doimp = F,doPCA = F,doGENE = F,whichpca = 1,
    #         univariate = F,dogeo = F,doChroms = F,
    #         whichchrom=chrom,matList=tempMatlist,spplist=spp,doEnvpcs=F)
    ...
    #         univariate = F,dogeo = F,doChroms = F,
    #         whichchrom=chrom,matList=tempMatlist,spplist=spp,doEnvpcs=F)
    # runGDMs(doimp = F,doPCA = F,doGENE = T,whichpca = 3,
    #         univariate = F,dogeo = F,doChroms = F,
    #         whichchrom=chrom,matList=tempMatlist,spplist=spp,doEnvpcs=F)
...
#   
#   runGDMs(doimp = F,doPCA = F,doGENE = F,whichpca = 1,
#           univariate = T,dogeo = F,doChroms = F,
#           whichchrom=chrom,matList,spplist=spp)
# }

# for (spp in spplist) {
#   runGDMs(doimp = F,doPCA = F,doGENE = F,whichpca = 1,
#           univariate = T,dogeo = T,doChroms = F,
#           whichchrom=chrom,matList,spplist=spp)
# runGDMs(doimp = F,doPCA = T,doGENE = F,whichpca = 1,
#         univariate = T,dogeo = T,doChroms = F,
#         whichchrom=chrom,matList,spplist=spp)
...
#runGDMs(doimp = F,doPCA = T,doGENE = F,whichpca = 1,
#        univariate = F,dogeo = F,doChroms = F,whichchrom=chromlist,matList,spplist)

print("allmorph without importance")

#runGDMs(doimp = F,doPCA = F,doGENE = F,whichpca = 1,univariate = F,dogeo = F,doChroms = F,
#        whichchrom="Z",matList,spplist)
...
# print(chrom)
#runGDMs(doimp = F,doPCA = F,doGENE = T,whichpca = 1,
#        univariate = F,dogeo = F,doChroms = T,whichchrom=chromlist,matList,spplist#=spp
#        ) 
## originally whichchrom=chrom 
# }
#}
...
"bil1a,bil16,
bru16,brumt,
cri1a,cri1b,cri2,cri4a,
cri5,cri6,cri7,cri8
cri9,cri16,cri17,cri18,
cri20,cri21,cri23,cri24,
cri25,cri26,cri28,crilg2,
crimt,
curALL,
fus1a,fus16,
mel16,melmt,
...
#           univariate = F,dogeo = F,doChroms = T,
#           whichchrom=chrom,matList,spplist=c("CURVIROSTRE"))
# }
#}
